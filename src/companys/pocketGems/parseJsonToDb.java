...
database格式
class Row {. from: 1point3acres.com/bbs
      String tableName;
      Map<String, String> map;
}
就是把json的格式parse了然后变成这样就行了，tableName就是对应上边的type（session，purchase，battle这种），map里就是每个event里data里所有的key value pair。. From 1point 3acres bbs
.1point3acres缃�
然后给了一个class
class JSONValue {
       String getAsString()
       Long getAsLong()
       Map getAsMap()
       List getAsList()-google 1point3acres
       ....
}
